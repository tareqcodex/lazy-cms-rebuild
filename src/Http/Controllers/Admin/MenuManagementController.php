...
class MenuManagementController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pages = Post::where('type', 'page')->latest()->take(20)->get();
        $posts = Post::where('type', 'post')->latest()->take(20)->get();
        ...
        foreach ($customPostTypes as $type) {
            $cptData[] = [
                'key' => 'cpt_' . $type->slug,
                'label' => $type->name,
                'items' => Post::where('type', $type->slug)->latest()->take(20)->get(),
                'type' => $type->slug
            ];
        }
        ...
        if ($menu) {
            ...
            $postIds     = $allItems->whereIn('type', ['post','page'])->pluck('object_id')->filter()->unique()->values()->all();
            $categoryIds = $allItems->where('type','category')->pluck('object_id')->filter()->unique()->values()->all();

            $existingPostIds     = $postIds     ? \Acme\CmsDashboard\Models\Post::withTrashed()->whereIn('id', $postIds)->pluck('id')->map(fn($id)=>(string)$id)->all()     : [];
            ...
            $buildItem = function($item, $depth) use ($existingPostIds, $existingCategoryIds) {
                $type      = $item->type ?? 'custom';
                $objectId  = $item->object_id ? (string)$item->object_id : null;
                $orphaned  = false;
                if (in_array($type, ['post','page']) && $objectId) {
                    $orphaned = !in_array($objectId, $existingPostIds);
