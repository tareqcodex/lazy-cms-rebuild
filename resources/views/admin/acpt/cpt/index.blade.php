    ...
    <div class="max-w-[1200px] mx-auto pb-12 mt-2">
        ...
        <div class="flex justify-between items-center mb-2">
            <div class="text-[13px] text-[#646970]">
                <a href="#" class="text-[#2271b1] font-semibold">All <span class="text-[#646970]">({{ $postTypes->count() }})</span></a> | 
                <a href="#" class="hover:text-[#2271b1]">Active <span class="text-[#646970]">({{ $postTypes->count() }})</span></a>
            </div>
                    ...
                    @forelse($postTypes as $type)
                        <tr class="group bg-white hover:bg-[#f6f7f7]">
                            ...
                            <td class="wp-table-cell text-[#2271b1] font-semibold text-[13px] py-2 relative flex-col">
                                <a href="{{ route('admin.acpt.cpt.edit', $type->id) }}" class="hover:underline">{{ $type->name }} {!! !$type->is_active ? '<span class="text-[#d63638] text-[11px] font-normal">(Inactive)</span>' : '' !!}</a>
                                <div class="invisible group-hover:visible mt-1 text-[13px] font-normal space-x-1 text-[#646970] flex items-center">
                                    ...
                                    <form action="{{ route('admin.acpt.cpt.toggle-status', $type->id) }}" method="POST" class="inline m-0 p-0 leading-none">
                                        @csrf
                                        <button type="submit" class="text-[#2271b1] hover:underline bg-transparent border-0 cursor-pointer p-0 leading-none">{{ $type->is_active ? 'Deactivate' : 'Activate' }}</button>
                                    </form> <span class="text-[#c3c4c7]">|</span> 
                                    
                                    <form action="{{ route('admin.acpt.cpt.destroy', $type->id) }}" method="POST" class="inline m-0 p-0 leading-none" onsubmit="return confirm('Are you sure you want to delete this CPT?');">
                                        @csrf
                                        @method('DELETE')
