<aside class="builder-sidebar flex flex-col" v-if="!isPreview">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar bg-white">
        ...
        <!-- Navigator Tab -->
        <div v-show="activeTab==='navigator'" class="animate-fade-in flex flex-col h-full overflow-hidden">
            <!-- Navigator Search -->
            <div class="px-4 py-3 border-b border-slate-50">
                <div class="relative group">
                    <i class="fa fa-search absolute left-3 top-1/2 -translate-y-1/2 text-[10px] text-slate-400 group-focus-within:text-[#0091ea] transition-colors"></i>
                    <input type="text" v-model="searchNavigatorQuery" 
                           placeholder="Filter elements..." 
                           class="w-full bg-slate-50 border border-slate-100 rounded-full py-1.5 pl-8 pr-4 text-[11px] focus:outline-none focus:border-[#0091ea] focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div v-else class="space-y-0.5">
                ...
                <div v-for="(cont, ci) in layout" :key="cont.id" 
                     v-show="!searchNavigatorQuery || 'container'.includes(searchNavigatorQuery.toLowerCase()) || cont.columns.some(col => 'column'.includes(searchNavigatorQuery.toLowerCase()) || col.elements.some(el => el.type.toLowerCase().includes(searchNavigatorQuery.toLowerCase())))"
                     class="group/nav">
                    <!-- Container Row -->
                    ...
                    <!-- Column Loop -->
                    <div v-for="(col, coli) in cont.columns" :key="col.id" 
                         v-show="!searchNavigatorQuery || 'column'.includes(searchNavigatorQuery.toLowerCase()) || col.elements.some(el => el.type.toLowerCase().includes(searchNavigatorQuery.toLowerCase()))"
                         class="ml-6 border-l border-slate-100">
                        ...
                        <!-- Elements Loop -->
                        <div v-for="(el, eli) in col.elements" :key="el.id" 
                             v-show="!searchNavigatorQuery || el.type.toLowerCase().includes(searchNavigatorQuery.toLowerCase())"
                             class="ml-6 border-l border-slate-50">
                            <!-- Standard Element -->
                            ...
                            <div v-else class="space-y-0.5">
                                ...
                                <!-- Nested Column Loop -->
                                <div v-for="(ncol, ncoli) in el.columns" :key="ncol.id" 
                                     v-show="!searchNavigatorQuery || 'nested column'.includes(searchNavigatorQuery.toLowerCase()) || ncol.elements.some(nel => nel.type.toLowerCase().includes(searchNavigatorQuery.toLowerCase()))"
                                     class="ml-6 border-l border-slate-100">
                                    <div class="flex items-center gap-2 px-4 py-1.5 hover:bg-slate-50 cursor-pointer group/line"
                                    ...
                                    <!-- Nested Elements -->
                                    <div v-for="(nel, neli) in ncol.elements" :key="nel.id" 
                                         v-show="!searchNavigatorQuery || nel.type.toLowerCase().includes(searchNavigatorQuery.toLowerCase())"
                                         class="ml-6 border-l border-slate-50">
